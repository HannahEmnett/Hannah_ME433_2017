...
void APP_Tasks(void) {
    ...
    switch (appData.state) {
        ...
        case APP_STATE_SCHEDULE_WRITE:
            ...
            i=0; 
            static char j=0, k=0, m=0;
            float w1=0.4, w2=0.6, f1=0.0102, f2=0.1177, f3=0.3721, f4=0.3721, f5=0.1177, f6=0.0102;
            float sum=0, maf=0, iir=0, fir=0;
            float buff[4];
            float buff2[6];
            if (i==0) {
                for (k=0; k<4; k++) buff[k]=0;
                for (k=0; k<6; k++) buff2[k]=0;
                k=0;
                sum=0;
            }
            _CP0_SET_COUNT(0);
            for (i=0; i<100; i++){
                ...
                for (j=2; j>=0; j--) buff[j+1]=buff[j];
                for (m=5; m>=0; m--) buff2[m+1]=buff2[m];
                buff[0]=accval[6]; 
                buff2[0]=accval[6];
                sum=0;
                iir=0;
                fir=0;
                for (k=0;k<4; k++) sum=sum+buff[k]; 
                maf=sum/4; 
                iir= (float) (buff[0]* w1 + buff[1]*w2); 
                fir = (float) (buff2[0]*f1 + buff2[1]*f2 + buff2[2]*f3 + buff2[3]*f4 + buff2[4]*f5 +buff2[5]*f6);
